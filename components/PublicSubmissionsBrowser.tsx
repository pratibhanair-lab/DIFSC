...
import PublicSubmissionDetailModal from "@/components/PublicSubmissionDetailModal";
...
export default function PublicSubmissionsBrowser({ submissions, categories }: Props) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--muted)", fontSize: 14, margin: "0 0 22px" }}>
        Every session and speaker suggestion received so far, across every review status. Click a row for full
        detail.
      </p>
      ...
      {modal && <PublicSubmissionDetailModal key={modal.id} submission={modal} onClose={() => setModalId(null)} />}
    </div>
  );
}
